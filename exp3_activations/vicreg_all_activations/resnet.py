...
class ResNet(nn.Module):
    def __init__(
        ...
        # we replace the first 7x7 Conv of stride 2 with 3x3 Conv of stride 1 per the SimCLR implementation
        # for the pre-training using CIFAR-10 because the images of CIFAR-10 are smaller than ImageNet
        ...
        self.relu = nn.ReLU(inplace=True)
        # Also remove the first maxpool layer because of the same reason
    ...
    def forward(self, x):
        ...
        # x = self.avgpool(x) # Removal of avg pool layer to obtain feature maps
        # x = torch.flatten(x, 1)
        # print("Feature maps shape: ", feature_maps.shape)
        return feature_maps


# create resnet18 with 512 output channels
def resnet18(**kwargs):
