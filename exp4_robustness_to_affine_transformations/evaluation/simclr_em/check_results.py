...
if __name__ == "__main__":
    ...
    device = args.device
    checkpoint = torch.load("check_results_checkpoints/simclr_linear_evaluation_after_pretrained_ckpt_epoch_435.pth.tar", map_location=device)  
    ...
    # finetuned test
    checkpoint = torch.load("check_results_checkpoints/finetune_final_mnist_acc92_ckpt_epoch_14.pth.tar", map_location=device)  
    model.load_state_dict(checkpoint['model_state'], strict=True)
    model = model.to(args.device)
