...
def get_arguments():
    ...
    # Running
    parser.add_argument("--num-workers", type=int, default=8) #It was 10 but I changed it bcs I was getting warnings.
...
class VICReg(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        # final embedding shape torch.Size([8, 800]) of mnist
        self.num_features = 800
        self.backbone, _ = resnet.__dict__[args.arch](
            zero_init_residual=True 
        )
        self.projection_head = resnet20(16, {'size': 40, 'channels': 1, 'classes': 10}, 32, 16, 1, mode="EM").to("cuda")
    ...
    def forward(self, x, y):
        ...
        cov_x = (x.T @ x) / (x.size(0) - 1) # (x.size(0) batch size)
        cov_y = (y.T @ y) / (x.size(0) - 1) # (x.size(0) batch size)
