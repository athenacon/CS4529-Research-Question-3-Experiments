...
class SimCLR(nn.Module):
    ...
    def forward(self, x_i):     
        # print("initial x_i", x_i.shape)
        x_i = self.encoder.conv1(x_i)
        # print("after conv1", x_i.shape)
        x_i = self.encoder.bn1(x_i)
        # print("after bn1", x_i.shape)
        x_i = self.encoder.relu(x_i)
        # print("after relu", x_i.shape)
        # print("x_i111", x_i) # but do we need maxpooling?
        # x_i = self.encoder.maxpool(x_i)
        # print("x_i2222", x_i)
        x_i = self.encoder.layer1(x_i)
        # print("after layer1", x_i.shape)
        # print("x2_i", x_i.shape)
